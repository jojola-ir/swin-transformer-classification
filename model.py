...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.shift_size > 0:
            ...
            # print(f"Shifted x shape : {shifted_x.shape}")
        else:
            shifted_x = x
...
def build_model(model_name, img_size=224, in_chans=3, embed_dim=96, depth=[2, 2, 2, 2], num_heads=[3, 6, 12, 24]):
    ...
    if model_name == "tiny":
        ...
        model = SUNet(img_size=img_size, patch_size=4, in_chans=in_chans, out_chans=in_chans,
                      embed_dim=embed_dim, depths=depth,
                      num_heads=num_heads,
                      window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=2,
                      drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1,
                      norm_layer=nn.LayerNorm, ape=False, patch_norm=True,
